...
class ImplicitNeuralRepresentation(nn.Module):
    ...
    def sample_grid(self, resolution=64, device='cpu', requires_grad=False):
        ...
        if requires_grad:
            # For optimization: need gradients
            occupancy = self.forward(grid).squeeze(-1)
        else:
            # For evaluation: no gradients needed
            with torch.no_grad():
                occupancy = self.forward(grid).squeeze(-1)
...
def optimize_single_subject(model, slices_2d, contours_2d, config, num_steps=200):
    ...
    for step in range(num_steps):
        ...
        # ====== FIX: Sample occupancy grid WITH gradient tracking ======
...
def evaluate_subject(model, occupancy_grid, contours_2d):
    ...
    # ====== FIX: Project 3D grid to 2D for comparison ======
    ...
    pred_projection_resized = F.interpolate(
        ...
        size=contours_2d[0].shape,  # (256, 256)
        mode='bilinear',
        align_corners=False
    ).squeeze()
