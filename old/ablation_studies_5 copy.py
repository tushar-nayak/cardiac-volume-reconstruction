...
def evaluate_reconstruction(model, contours_2d, device, resolution=64):
    ...
    with torch.no_grad():
        ...
        # Resize to match target contour size
        ...
        for val in parameter_values:
            ...
            try:
                ...
                model, _ = optimize_single_subject(
                    model, slices, contours, cfg, num_steps=50
                )
                ...
                print(f"    Error {parameter_name}={val}: {type(e).__name__}")
                results[parameter_name].append(val)
...
def main():
    print("="*60)
    print("ABLATION STUDIES - FINAL VERSION")
    print("="*60)
