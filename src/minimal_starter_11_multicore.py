...
CONFIG: Dict = {
    ...
    # parallelism
    "num_workers": 8,  # >1 to enable parallel per-scan processing
...
def main() -> None:
    ...
    if num_workers > 1 and "cuda" in str(CONFIG["device"]).lower():
        print(
            f"NOTE: Running {num_workers} workers on GPU. "
            "Monitor GPU memory usage; reduce num_workers if you see OOM."
        )
